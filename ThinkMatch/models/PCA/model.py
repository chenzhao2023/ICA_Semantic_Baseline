...
class Net(CNN):
    def __init__(self):
        super(Net, self).__init__()  # FEATURE_CHANNEL = 512, GNN_FEAT = 2048
    ...
    def forward(self, data_dict, **kwargs):
        if 'images' in data_dict:
            ...
            # extract feature, node_feat_map_size = 32, edge_feat_map_size = 16
            src_node = self.node_layers(src) # src [batch, feature_dim, node_feat_map_size, node_feat_map_size]
            src_edge = self.edge_layers(src_node) #[batch, feature_dim, edge_feat_map_size, edge_feat_map_size]
            tgt_node = self.node_layers(tgt) # tgt [batch, feature_dim, node_feat_map_size, node_feat_map_size]
            tgt_edge = self.edge_layers(tgt_node) #[batch, feature_dim, edge_feat_map_size, edge_feat_map_size]
            ...
            # arrange features
            U_src = feature_align(src_node, P_src, ns_src, self.rescale) # node features for g1, [batch, feature_dim, n_node]
            F_src = feature_align(src_edge, P_src, ns_src, self.rescale) # edge features for g1, [batch, feature_dim, n_node]
            U_tgt = feature_align(tgt_node, P_tgt, ns_tgt, self.rescale) # node features for g2, [batch, feature_dim, n_node]
            F_tgt = feature_align(tgt_edge, P_tgt, ns_tgt, self.rescale) # edge features for g2, [batch, feature_dim, n_node]
            ...
            U_src = src[:, :src.shape[1] // 2, :] # batch_size, feature dimension, feature dimension 2
            F_src = src[:, src.shape[1] // 2:, :] # batch_size, feature dimension, feature dimension 2
            U_tgt = tgt[:, :tgt.shape[1] // 2, :] # batch_size, feature dimension, feature dimension 2
            F_tgt = tgt[:, tgt.shape[1] // 2:, :] # batch_size, feature dimension, feature dimension 2
        else:
            raise ValueError('Unknown data type for this model.')

        emb1 = torch.cat((U_src, F_src), dim=1).transpose(1, 2) # concat([batch, feat_dim, n_node], [batch, feat_dim, n_node])
        emb2 = torch.cat((U_tgt, F_tgt), dim=1).transpose(1, 2)
