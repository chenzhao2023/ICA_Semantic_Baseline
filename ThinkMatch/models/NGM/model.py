...
class Net(CNN):
    ...
    def forward(self, data_dict, **kwargs):
        ...
        if 'images' in data_dict:
            ...
            # extract feature
            src_node = self.node_layers(src)          # batch_size*512*32*32
            src_edge = self.edge_layers(src_node)     # batch_size*512*16*16
            tgt_node = self.node_layers(tgt)          # batch_size*512*32*32
            tgt_edge = self.edge_layers(tgt_node)     # batch_size*512*16*16
            ...
            # arrange features
            U_src = feature_align(src_node, P_src, ns_src, self.rescale) # batch_size*512*10
            F_src = feature_align(src_edge, P_src, ns_src, self.rescale) # batch_size*512*10
            U_tgt = feature_align(tgt_node, P_tgt, ns_tgt, self.rescale) # batch_size*512*10
            F_tgt = feature_align(tgt_edge, P_tgt, ns_tgt, self.rescale) # batch_size*512*10
            ...
            U_src = src[:, :src.shape[1] // 2, :] # 512*10
            F_src = src[:, src.shape[1] // 2:, :] # 512*10
            U_tgt = tgt[:, :tgt.shape[1] // 2, :] # 512*10
            F_tgt = tgt[:, tgt.shape[1] // 2:, :] # 512*10
        ...
        if 'images' in data_dict or 'features' in data_dict:
            ...
            if cfg.NGM.EDGE_FEATURE == 'cat':
                X = reshape_edge_feature(F_src, G_src, H_src) # 1024*edge_num
                Y = reshape_edge_feature(F_tgt, G_tgt, H_tgt) # 1024*edge_num
            elif cfg.NGM.EDGE_FEATURE == 'geo':
