...
"""
reference: https://github.com/SreenivasVRao/ConvGRU-ConvLSTM-PyTorch
"""
...
if __name__ == "__main__":
    ...
    x1 = torch.randn([5, 32, 60, 60]).cuda() # batch, feat_dim, im_size, im_size
    x2 = torch.randn([5, 32, 60, 60]).cuda()
    x3 = torch.randn([5, 32, 60, 60]).cuda()
    
    clstm = ConvLSTM(in_channels=32, hidden_channels=64, kernel_size=(3, 3), num_layers=1, batch_first=True).cuda()
    ...
    out = cblstm(x_fwd, x_rev) # batch,seq_len,channel,height,width
    print (out.shape)
