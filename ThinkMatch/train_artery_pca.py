...
class ArteryDatasetPCA(Dataset):
    def __init__(self, dataset, samples, rand, cache=False, cache_path=".cache"):
    ...
    def generate_pair(g0, g1):
        ...
        A0 = np.array(nx.adjacency_matrix(g0).todense(), dtype=np.float32)
        A1 = np.array(nx.adjacency_matrix(g1).todense(), dtype=np.float32)
        ...
        feat1 = np.stack([np.array(g1.nodes()[i]['data'].features, dtype=np.float32) for i in range(n1)], axis=-1).T
...
class PCA_Model_Trainer(object):
    ...
    def __init_dataset__(self):
        ...
        dataloader_train = ArteryDatasetPCA(dataset_train, self.sample_train, self.rand, cache=self.params.cache, cache_path=self.params.cache_path)
        self.dataloaders = {}
        ...
        for i in tqdm(range(len(self.sample_test))):
            for j in range(len(self.sample_template)):
                ...
                if g0_category == g1_category and n0 <= n1:
                    inputs = ArteryDatasetPCA.generate_pair(g0, g1)
                    inputs = data_to_cuda(inputs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='/media/z/data2/artery_semantic_segmentation/ThinkMatch/artery/data2')
    parser.add_argument('--cache', type=bool, default=True)
    parser.add_argument('--cache_path', type=str, default='/media/z/data2/artery_semantic_segmentation/ThinkMatch/cache/pca')
    ...
    # flow control
    parser.add_argument('--flow', type=str, default="complexity")

    args = parser.parse_args()

    if args.flow == "complexity":
