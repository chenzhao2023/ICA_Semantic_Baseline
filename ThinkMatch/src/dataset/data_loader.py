...
class GMDataset(Dataset):
    ...
    #     self.__init__(self=self, name=name, bm=bm, length=length,
    #                   using_all_graphs=using_all_graphs, cls=clz, problem=problem)
    #
    #     #super(GMDataset).__new__(kwargs)
    #     return super().__new__()

    def __init__(self, name, bm, length, using_all_graphs=False, clz=None, problem='2GM'):
        self.name = name
        self.bm = bm
        self.using_all_graphs = using_all_graphs
        self.obj_size = self.bm.obj_resize
        self.test = True if self.bm.sets == 'test' else False
        self.clz = None if clz in ['none', 'all'] else clz
        ...
        if self.problem_type == '2GM':
            self.id_combination, self.length = self.bm.get_id_combination(self.clz)
            self.length_list = []
    ...
    def __getitem__(self, idx):
        if self.problem_type == '2GM':
            return self.get_pair(idx, self.clz)
        elif self.problem_type == 'MGM':
            return self.get_multi(idx, self.clz)
    ...
    def get_pair(self, idx, cls):
        ...
        P1 = [(kp['x'], kp['y']) for kp in anno_pair[0]['kpts']]  # key points
        P2 = [(kp['x'], kp['y']) for kp in anno_pair[1]['kpts']]  # key points
        ...
        A1, G1, H1, e1 = build_graphs(P1, n1, stg=cfg.GRAPH.SRC_GRAPH_CONSTRUCT, sym=cfg.GRAPH.SYM_ADJACENCY) # A1=G1*H1.T
    ...
    def get_multi_cluster(self, idx):
        dicts = []
        if self.clz is None or self.clz == 'none':
            cls_iterator = random.choice(self.classes)
        else:
            cls_iterator = self.clz
