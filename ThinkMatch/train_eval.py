...
import argparse
from src.utils.config import cfg_from_file, cfg_from_list, get_output_dir
                ...
                with torch.set_grad_enabled(True):
                    ...
                    if cfg.PROBLEM.TYPE == '2GM':
                        ...
                        if cfg.TRAIN.LOSS_FUNC in ['perm', 'ce' 'hung']:
                            ...
                            raise ValueError(
                                'Unsupported loss function {} for problem type {}'.format(cfg.TRAIN.LOSS_FUNC, cfg.PROBLEM.TYPE))
...
def parse_args(description):
    ...
    parser.add_argument('--cfg', '--config', dest='cfg_file', action='append',
                        help='an optional config file', default=['experiments/vgg16_pca_voc.yaml'], type=str)
...
if __name__ == '__main__':
    ...
    cfg = parse_args('Deep learning of graph matching training & evaluation code.')
    ...
    benchmark = {
        x: Benchmark(name=cfg.DATASET_FULL_NAME, sets=x, problem=cfg.PROBLEM.TYPE,
                     obj_resize=cfg.PROBLEM.RESCALE, filter=cfg.PROBLEM.FILTER, **ds_dict)
        for x in ('train', 'test')}

    train_dataset = GMDataset(name=cfg.DATASET_FULL_NAME, 
                              bm=benchmark['train'], 
